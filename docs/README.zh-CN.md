...
> English version: [README.md](../README.md) / English docs are at the repository root.
...
> 遵循 [GNU 通用公共许可证 v3.0 only](../LICENSE) 授权。

TomoRead 是一款基于 Flutter 的跨平台 AI 阅读器，面向本地 EPUB/PDF/TXT/Markdown 书库、沉浸式阅读、全局知识整理和基于原文的 AI 对话。

书籍、阅读位置、书签、标注、笔记、对话和阅读统计默认保存在本地；模型 API Key 通过系统安全存储管理。使用 AI 功能时，用户输入和明确附加的原文会发送到所配置的模型服务商。
...
- Android（实验性）：APK 分为 `armeabi-v7a`（32 位 ARM）和 `arm64-v8a`（64 位 ARM），AAB 用于应用商店
...
├── features/
│   ├── library/            # 书库与书籍详情
│   ├── reader/             # EPUB/PDF/TXT/Markdown 阅读工作区
│   ├── chat/               # AI 对话
│   ├── notes/              # 全局笔记
│   ├── statistics/         # 阅读统计
│   ├── settings/           # 软件与阅读设置
...
- [核心架构](architecture.md)：本地数据、AI 对话、全局笔记与阅读统计的边界。
- [阅读能力与数据边界](reader-features.md)：当前格式能力、定位、标注、显示投影与内容安全规则。
- [产品路线图](roadmap.md)：以 ColorTxt、ReadAny 为参照的后续优先级与交付原则。
...
- [x] 本地 EPUB/PDF/TXT/Markdown 书库与书籍详情
